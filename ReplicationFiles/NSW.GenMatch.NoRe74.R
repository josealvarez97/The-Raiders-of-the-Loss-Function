...
# based on a NSW "faked" obs data set when re74 is not included.
# (aka, for the original LALONDE SAMPLE)

# and defines

# NSW.MeasureBalance.NoRE74 = function(nsw_obs_data, match.output)
# which returns the output MatchBalance, based on the output of NSW.GenMatch.NoRE74()
# similarly, it works for a NSW "faked" obs data set when re74 is not included.
...
NSW.GenMatch.NoRE74 = function(nsw_obs_data, population = 100, max.gen = 100, wait.gen = 4, my.loss.f = 1){
  ...
  X = as.matrix(cbind(
    ...
    nsw_obs_data$married, nsw_obs_data$nodegree, #nsw_obs_data$re74, 
    nsw_obs_data$re75
  ))
...
# Lalonde Original Dataset (Does not include RE74)
#Lalonde.mout = NSW.GenMatch.NoRE74(lalonde.nsw_treated_data_with_CPS1, population = 600, max.gen = 50, wait.gen = 30, my.loss.f = my.loss.function.9.02)
#Lalonde.balout = NSW.MeasureBalance.NoRE74(lalonde.nsw_treated_data_with_CPS1, Lalonde.mout)

#summary(Lalonde.mout)
